...
Commonlib.runIfEnableAndLoad('add-tweet-button', async () => {
  ...
  const text =
    navigator.language === 'ja' ? '最新のコンテスト結果をツイート' : 'Tweet the result of the latest contest';
  const $tweetButton = $('<a>')
    .addClass('tweet')
    .text(text)
    ...
    .prop('target', '_blank');
  if (isDetailPage) {
    $('#history_wrapper > div.row:first-child > .col-sm-6:first-child')
      .eq(0)
      .prepend($tweetButton);
  }
});
